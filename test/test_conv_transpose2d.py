...
test_data = [(batch_size, height, width, in_channels, out_channels, kernel_size_h, kernel_size_w, in_length, out_length,
              stride, padding, dilation, routing_type, kwargs, num_iterations) for batch_size in [1, 3] for height in
             ...
             ['dynamic', 'k_means'] for kwargs in kwargs_data[routing_type] for num_iterations in [1, 3, 4]]


@pytest.mark.parametrize('batch_size, height, width, in_channels, out_channels, kernel_size_h, kernel_size_w, '
                         'in_length, out_length, stride, padding, dilation, routing_type, kwargs, num_iterations',
                         test_data)
