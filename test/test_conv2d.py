...
test_data = [(batch_size, height, width, in_channels, out_channels, kernel_size_h, kernel_size_w, in_length, out_length,
             ...
             [5, 12] for in_length in [1] for out_length in [1, 3] for kernel_size_h in [1, 3] for kernel_size_w in
             [1, 3] for in_channels in [1 * in_length, 3 * in_length] for out_channels in
             [1 * out_length, 3 * out_length] for stride in [1, 2] for padding in [0, 1] for routing_type in ['sum'] for
             num_iterations in [1, 3]]
...
def test_function(batch_size, height, width, in_channels, out_channels, kernel_size_h, kernel_size_w, in_length,
    ...
    y_fast = CL.capsule_cov2d(x_gpu, w_gpu, out_length=out_length, stride=stride, padding=padding,
                              routing_type=routing_type, num_iterations=num_iterations)
    y_ref = CL.capsule_cov2d(x_cpu, w_cpu, out_length=out_length, stride=stride, padding=padding,
                             routing_type=routing_type, num_iterations=num_iterations)
...
def test_multigpu(batch_size, height, width, in_channels, out_channels, kernel_size_h, kernel_size_w, in_length,
    ...
    y0 = CL.capsule_cov2d(a0, w0, out_length=out_length, stride=stride, padding=padding, routing_type=routing_type,
                          num_iterations=num_iterations)
    go = torch.randn(y0.size()).cuda()
    y0.backward(go)
    y1 = CL.capsule_cov2d(a1, w1, out_length=out_length, stride=stride, padding=padding, routing_type=routing_type,
                          num_iterations=num_iterations)
