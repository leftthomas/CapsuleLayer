...
def test_function(routing_type, num_iterations):
    ...
    w_cpu = Variable(torch.randn(5, 3, 3, 2, 1, 4).double(), requires_grad=True)
    ...
    assert gradcheck(
        partial(CL.capsule_cov2d, stride=1, padding=1, routing_type=routing_type, num_iterations=num_iterations),
        (x_gpu, w_gpu))
...
def test_module(routing_type, num_iterations):
    module = CapsuleConv2d(in_channels=3, out_channels=8, kernel_size=3, in_length=1, out_length=4, stride=1,
                           padding=1, routing_type=routing_type, num_iterations=num_iterations)
    x = Variable(torch.randn(4, 3, 6, 11))
    y_cpu = module(x)
...
def test_multigpu(routing_type, num_iterations):
    ...
    w0 = Variable(torch.randn(4, 3, 3, 2, 1, 8).double().cuda(0), requires_grad=True)
    w1 = Variable(torch.randn(4, 3, 3, 2, 1, 8).double().cuda(1), requires_grad=True)
    y0 = CL.capsule_cov2d(a0, w0, stride=1, padding=1, routing_type=routing_type, num_iterations=num_iterations)
    go = torch.randn(y0.size()).double().cuda()
    y0.backward(go)
    y1 = CL.capsule_cov2d(a1, w1, stride=1, padding=1, routing_type=routing_type, num_iterations=num_iterations)
